codeunit 60301 "NLOutlookExtension"
{
    ...
    begin
        ...
        Contact.Validate("E-Mail", InvoiceEmail);

        if not Contact.Insert() then
            Error('Error creating company contact.');

        // Create the primary contact person
        PrimaryContactNo := RegHelper.NewContact(
            ContactNoForCompany,
            PrimaryContactFirstAndLastName,
            PrimaryContactDirectPhoneNumber,
            PrimaryContactMobilePhoneNumber,
            PrimaryContactEmail,
            PrimaryContactTitle
        );
